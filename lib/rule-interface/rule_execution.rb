module RuleInterface
  class RuleExecution
    class << self
      ...
      def execute(data_hash:, type:, container:, package:)
        payload = Service::Converter.hash_to_drool(data_hash: data_hash, type: type, package: package)
        response = parsed_connection(container: container, method: "post", payload: payload)
        if response[:type] && response[:type] == "SUCCESS"
          Service::Converter.drool_to_hash(response[:result][:"execution-results"][:results])
        else
